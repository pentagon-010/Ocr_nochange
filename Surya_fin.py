...
def save_ocr_to_pdf(all_bboxes, pdf_filename: str):
    ...
    for img_size, bboxes, text_lines in all_bboxes:
        ...
        for bbox, text in zip(bboxes, text_lines):
            x0, y0, x1, y1 = bbox
            # Normalize coordinates to fit PDF dimensions
            norm_x = x0 / img_width
            norm_y = y0 / img_height
            pdf_x = norm_x * pdf_width
            pdf_y = (1 - norm_y) * pdf_height  # Flip Y-axis for PDF rendering
            pdf.drawString(pdf_x, pdf_y, text)

        pdf.showPage()  # End current page and start a new one
...
st.markdown("""
# OCR Demo with PDF Export

This app performs OCR for all pages in a PDF and exports results to a PDF file.
...
if st.sidebar.button("Run OCR"):
    ...
    save_ocr_to_pdf(all_bboxes, pdf_filename)
